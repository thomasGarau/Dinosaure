...
import { Level1, LevelState } from "./LevelState.js";

const level: Level1 = new Level1();
let dino: Dinosaure = new Dinosaure(level);
let env: Environement = new Environement(level);
const canvas: HTMLCanvasElement = document.querySelector('#monCanvas') as HTMLCanvasElement;
...
function displayEndGame(){
    jeuTermine = true;
    const scoreMax: number = parseInt(localStorage.getItem('scoreMax') ?? "0");
    if(dino.scoreDino > scoreMax){
        localStorage.setItem('scoreMax', dino.scoreDino.toString());
    }
    ctx.drawImage(
        spriteSheet,
        1293, // Coordonnée X de départ sur l'image source
    ...
    ctx.drawImage(
        spriteSheet,
        0, // Coordonnée X de départ sur l'image source
